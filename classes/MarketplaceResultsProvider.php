    ...
    public function getResultsHtml($page, $pageSize, $term) {
        try {
            ...
            $query = $this->con->prepare("SELECT * 
                                         ...
                                         OR description LIKE :term 
                                         OR keywords LIKE :term
                                         ORDER BY clicks, isFeatured DESC
            ...
            while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
                $id = $row["id"];
                $userId = $row["userId"]; 
                $dataUserId = htmlspecialchars($userId, ENT_QUOTES, 'UTF-8');
                $title = $row["title"];
                ...
                $resultsHtml .= " &nbsp;<a href='#' class='see-market-image' onClick='getProductPictures($id)'>
